...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QCheckBox, QMessageBox, QDockWidget, QScrollArea, QSizePolicy, QFileDialog, QButtonGroup)
from PySide6.QtMultimedia import QMediaDevices
from PySide6.QtCore import QEvent, QTimer, Qt, QThread, Signal, Slot
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.dock_tuning, tuning_content = self.create_dock("Tuning & Source", "DockTuning", Qt.LeftDockWidgetArea)
        ...
        self.dock_eye = QDockWidget("Eye View", self)
        ...
        self.eye_video_label = QLabel("Eye View")
        ...
        self.eye_video_label.setMinimumSize(320, 240)
        self.eye_video_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.dock_head = QDockWidget("Head View", self)
        ...
        self.head_video_label = QLabel("Head View")
        self.head_video_label.setAlignment(Qt.AlignCenter)
        self.head_video_label.setStyleSheet("background-color: black; color: white;")
        # Fix: Ensure Minimum Size so dock doesn't collapse
        self.head_video_label.setMinimumSize(320, 240)
        self.head_video_label.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        source_group = QGroupBox("Device Sources")
        source_layout = QFormLayout()

        self.scan_cam_btn = QPushButton("Refresh Device List")
        ...
        self.cam_mode_combo.addItem("Auto (Driver Default)", "Auto")
        self.cam_mode_combo.addItem("High FPS (320x240 YUY2 120fps)", "low")
        self.cam_mode_combo.addItem("Standard (640x480 MJPG 30fps)", "std")
        self.cam_mode_combo.addItem("High Res (1280x720 MJPG 30fps)", "high")
        self.cam_mode_combo.setCurrentIndex(0) 

        self.scene_enable_checkbox = QCheckBox("Enable Scene Camera")
        self.scene_enable_checkbox.setChecked(True)

        source_layout.addRow(self.scan_cam_btn)
        source_layout.addRow("Eye Source:", self.eye_cam_combo)
        source_layout.addRow("Eye Mode:", self.cam_mode_combo)
        source_layout.addRow("Scene Source:", self.scene_cam_combo)
        source_layout.addRow("", self.scene_enable_checkbox)
        source_group.setLayout(source_layout)
        tuning_layout.addWidget(source_group)
        
        # --- GROUP 2: IMAGE TUNING ---
        tuning_group = QGroupBox("Image Tuning")
        tuning_layout_inner = QFormLayout()
        ...
        tuning_group.setLayout(tuning_layout_inner)
        tuning_layout.addWidget(tuning_group)

        # --- GROUP 3: TRACKING SETTINGS ---
        feature_group = QGroupBox("Tracking Settings")
        feature_layout = QFormLayout()

        # Eye Start
        self.start_cam_btn = QPushButton("Start Eye/Scene")
        self.start_cam_btn.clicked.connect(self.toggle_camera)

        # Head Tracking
        self.head_enable_check = QCheckBox("Enable Head Tracker")
        ...
        self.btn_eye_only = QPushButton("Eye Only")
        self.btn_eye_only.setCheckable(True)
        self.btn_eye_only.setChecked(True)
        self.btn_head_only = QPushButton("Head Only")
        ...
        tuning_layout.addWidget(feature_group)

        # --- GROUP 4: GLOBE PARAMETERS (Advanced) ---
        model_group = QGroupBox("Globe Parameters")
        ...
        tuning_layout.addWidget(model_group)
        ...
        self.pupil_status_lbl = QLabel("Pupil: Not Detected")
        self.pupil_status_lbl.setStyleSheet("background-color: #333; color: #ff4d4d; font-weight: bold; border-radius: 4px; padding: 2px;")
        self.pupil_status_lbl.setAlignment(Qt.AlignCenter)
        eye_layout.addWidget(self.pupil_status_lbl)
        ...
        self.auto_fit_btn = QPushButton("Auto-Fit Globe")
        self.auto_fit_btn.clicked.connect(self.start_auto_fit_globe)
        self.auto_fit_btn.setStyleSheet("background-color: #209020; font-weight: bold;") 
        
        self.reset_globe_btn = QPushButton("Reset Globe")
        self.reset_globe_btn.clicked.connect(self.reset_globe_data)
        self.reset_globe_btn.setStyleSheet("background-color: #D0504F; font-weight: bold;")
        ...
        self.cal_btn_grid = QPushButton("Calibrate 9 Points")
        ...
        workflow_layout.addWidget(cal_group)
        
        # 3. Session & Media
        sess_group = QGroupBox("Session & Media")
        sess_layout = QVBoxLayout()
        
        # Media Stimulus
        media_layout = QHBoxLayout()
        self.load_media_btn = QPushButton("Load Media")
        ...
        self.btn_media_play = QPushButton("Play") # Play/Pause toggle
        self.btn_media_stop = QPushButton("Stop")
        self.btn_media_replay = QPushButton("Replay")
        ...
        # Help/Workflow Button
        self.btn_help = QPushButton("📖 Help / Workflow")
        self.btn_help.clicked.connect(self.show_help_dialog)
        self.btn_help.setStyleSheet("background-color: #1565C0; color: white; font-weight: bold;")
        sess_layout.addWidget(self.btn_help)
        
        # Calibration Media Button
        self.cal_btn_media = QPushButton("Calibrate 9 Points (Media)")
        ...
        sess_layout.addWidget(self.cal_btn_media)
        ...
        self.sess_info_lbl = QLabel("Time: 0.0s\nSamples: 0")
        
        # Options - Recording
        self.check_video = QPushButton("Record Video: Off")
        self.check_video.setCheckable(True)
        self.check_video.clicked.connect(self.toggle_video_option)
        
        self.check_path = QPushButton("Show Gaze Path: Off")
        ...
        self.check_gaze_cursor = QPushButton("👁 Show Gaze Cursor: Off")
        ...
        # Heatmap controls
        self.check_heatmap = QPushButton("🔥 Show Heatmap: Off")
        ...
        # Heatmap Config Layout
        hm_config_layout = QHBoxLayout()
        
        # Resolution Combo
        self.combo_heatmap_res = QComboBox()
        self.combo_heatmap_res.addItems(["0.5K (Fast)", "1K (SD)", "2K (FHD)", "4K (UHD)"])
        self.combo_heatmap_res.currentIndexChanged.connect(self.update_heatmap_res)
        self.combo_heatmap_res.setToolTip("Proxy Resolution for Preview (Export uses Scale)")
        
        # Blur Slider
        self.lbl_blur = QLabel("Blur: 31")
        self.slider_heatmap_blur = QSlider(Qt.Horizontal)
        self.slider_heatmap_blur.setRange(5, 150)
        self.slider_heatmap_blur.setValue(31)
        self.slider_heatmap_blur.valueChanged.connect(self.update_heatmap_blur)
        
        hm_config_layout.addWidget(QLabel("Res:"))
        hm_config_layout.addWidget(self.combo_heatmap_res)
        hm_config_layout.addWidget(self.lbl_blur)
        hm_config_layout.addWidget(self.slider_heatmap_blur)
        ...
        sess_layout.addLayout(smooth_layout) # Added Smoothing here
        sess_layout.addWidget(self.check_heatmap)
        sess_layout.addWidget(self.btn_clear_heatmap)
        sess_layout.addWidget(self.sess_info_lbl)
        sess_group.setLayout(sess_layout)
        workflow_layout.addWidget(sess_group)
        ...
        self.exp_json_btn = QPushButton("Export JSON (nTop)")
        self.exp_json_btn.clicked.connect(self.export_json)
        self.exp_obj_btn = QPushButton("Export OBJ (Mesh)")
        self.exp_obj_btn.clicked.connect(self.export_obj)
        self.exp_heatmap_btn = QPushButton("📸 Export Heatmap PNG")
        self.exp_heatmap_btn.clicked.connect(self.export_heatmap_png)
        self.exp_heatmap_btn.setStyleSheet("background-color: #B71C1C; color: white;")
        ...
        # eye_layout.addStretch() # Don't stretch eye so it stays centered/top or expands
        workflow_layout.addStretch()
        
        # Signature & Credits
        credit_label = QLabel("vibe code by lyes hammadouche on a fork from Orlosky\nSource: github.com/JEOresearch/EyeTracker")
        credit_label.setAlignment(Qt.AlignCenter)
        credit_label.setStyleSheet("color: #888; font-size: 10px; margin-top: 15px; border-top: 1px solid #444; padding-top: 10px;")
        workflow_layout.addWidget(credit_label)
    ...
    def update_mapping_mode(self, index):
        ...
        if data.startswith("screen_"):
            ...
            if idx < len(self.screens_info):
                ...
                # Force show on that screen
                self.calibration_overlay.windowHandle().setScreen(screen)
                self.calibration_overlay.showFullScreen() # or show() with geometry
    ...
    def on_pupil_data(self, pupil_data):
        ...
        if self.current_pupil and self.current_pupil.get('confidence', 0) > 0.1:
            ...
            self.pupil_status_lbl.setStyleSheet("background-color: #333; color: #00ff00; font-weight: bold; border-radius: 4px; padding: 2px;")
        else:
    ...
    def _load_gaze_cursor_image(self):
        ...
        path_long = os.path.join(base_path, "interface elements", "Eye-fb-oval.png")
    ...
    def update_frame(self):
        ...
        if scene_frame is None:
            ...
            cv2.putText(scene_frame, "No Scene Camera", (200, 240), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        else:
            if not self.media_mode:
        ...
        self.video_label.setPixmap(QPixmap.fromImage(qt_scene).scaled(self.video_label.size(), Qt.KeepAspectRatio))
    ...
    def draw_scene_overlay(self, frame):
        ...
        # Draw Auto-Fit Instructions with COLOR-CODED PROGRESS BARS
        if self.is_fitting_globe:
            elapsed = time.time() - self.fit_start_time
            ...
            confidence_pct = 0
            if total_samples >= 15:
                confidence_pct = min(50, (total_samples / 60.0) * 50)  # 50% from count
            if angle_coverage >= 30:
                confidence_pct += min(50, (angle_coverage / 80.0) * 50)  # 50% from coverage
            
            # Draw dark background panel
            panel_h = 160
            cv2.rectangle(frame, (20, 180), (620, 180 + panel_h), (0, 0, 0), -1)
            cv2.rectangle(frame, (20, 180), (620, 180 + panel_h), (100, 100, 100), 2)
            
            # Title
            cv2.putText(frame, "CALIBRATING GLOBE - MOVE EYES RANDOMLY", 
                        (40, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 255, 255), 2)
            ...
            # Sample count: 0-60 samples (green at 60+)
            sample_progress = min(1.0, total_samples / 60.0)
            sample_bar_fill = int(bar_width * sample_progress)
            sample_color = self._get_quality_color(total_samples, 15, 50)
            
            # Background
            cv2.rectangle(frame, (40, bar_y), (40 + bar_width, bar_y + bar_height), (50, 50, 50), -1)
            # Fill
            if sample_bar_fill > 0:
                cv2.rectangle(frame, (40, bar_y), (40 + sample_bar_fill, bar_y + bar_height), sample_color, -1)
            # Label
            cv2.putText(frame, f"Samples: {total_samples}/60", 
                        (45, bar_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1)
            
            # === ANGLE COVERAGE BAR ===
            bar_y = 270
            
            # Coverage: 0-100% (green at 80%+)
            coverage_progress = angle_coverage / 100.0
            coverage_bar_fill = int(bar_width * coverage_progress)
            coverage_color = self._get_quality_color(angle_coverage, 30, 75)
            
            # Background
            cv2.rectangle(frame, (40, bar_y), (40 + bar_width, bar_y + bar_height), (50, 50, 50), -1)
            # Fill
            if coverage_bar_fill > 0:
                cv2.rectangle(frame, (40, bar_y), (40 + coverage_bar_fill, bar_y + bar_height), coverage_color, -1)
            # Label
            cv2.putText(frame, f"Coverage: {angle_coverage:.0f}%", 
                        (45, bar_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1)
            
            # === OVERALL CONFIDENCE BAR ===
            bar_y = 310
            
            # Overall: 0-100% (green at 80%+)
            conf_progress = confidence_pct / 100.0
            conf_bar_fill = int(bar_width * conf_progress)
            conf_color = self._get_quality_color(confidence_pct, 30, 75)
            
            # Background
            cv2.rectangle(frame, (40, bar_y), (40 + bar_width, bar_y + bar_height), (50, 50, 50), -1)
            # Fill
            if conf_bar_fill > 0:
                cv2.rectangle(frame, (40, bar_y), (40 + conf_bar_fill, bar_y + bar_height), conf_color, -1)
            # Label
            cv2.putText(frame, f"Confidence: {confidence_pct:.0f}%", 
                        (45, bar_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200, 200, 200), 1)
            
            # Timer at bottom
            cv2.putText(frame, f"Time Remaining: {remaining:.1f}s", 
                        (40, bar_y + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (150, 150, 255), 1)
        ...
        if self.is_calibrating:
            if self.cal_mode == "center":
                if self.center_cal_step == 1:
                    ...
                    msg = "Step 2: Align scene center to screen center, look at dot -> Press 'C'"
            else:
                msg = f"Look at Target {self.calibration_step + 1}/9 and Press 'C'"
            ...
            if self.cal_mode == "center":
                if self.center_cal_step == 2:
                    ...
                    cv2.line(frame, (px, py-line_len), (px, py+line_len), (0, 0, 255), thickness)
            else:
        ...
        if self.scene_gaze and self.show_gaze_cursor:
            ...
            font_scale = 0.7 * max(1, w / 640)
            text_y = int(h - 30)
            if self.canvas_gaze:
                cmx, cmy = self.canvas_gaze
                cv2.putText(frame, f"Gaze: {cmx:.1f}, {cmy:.1f} mm", (20, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (0, 255, 255), max(1, int(2 * max(1, w / 640))))
    ...
    def capture_calibration_point(self):
        ...
        if self.cal_mode == "center":
            if self.center_cal_step == 1:
                self.center_cal_step = 2
                self.cal_status_lbl.setText("Step 2: Align scene center to screen center, look at dot -> Press 'C'")
                return
            if self.center_cal_step == 2:
    ...
    def toggle_session(self):
        if not self.session.is_running:
            ...
            self.start_sess_btn.setText("Start Session")

    def toggle_video_option(self):
        self.record_video = self.check_video.isChecked()
        state = "On" if self.record_video else "Off"
        self.check_video.setText(f"Record Video: {state}")
        
    def toggle_path_option(self):
        self.show_gaze_path = self.check_path.isChecked()
        state = "On" if self.show_gaze_path else "Off"
        self.check_path.setText(f"Show Gaze Path: {state}")
    ...
    def toggle_heatmap_option(self):
        ...
        self.check_heatmap.setText(f"🔥 Show Heatmap: {state}")
        self.check_heatmap.setText(f"🔥 Show Heatmap: {state}")
    ...
    def update_heatmap_blur(self, value):
        ...
        self.lbl_blur.setText(f"Blur: {value}")
        # Reset cache
        self.heatmap_overlay_cache = None
    
    def toggle_gaze_cursor_option(self):
        self.show_gaze_cursor = self.check_gaze_cursor.isChecked()
        state = "On" if self.show_gaze_cursor else "Off"
        self.check_gaze_cursor.setText(f"👁 Show Gaze Cursor: {state}")
    ...
    def export_json(self):
        ...
        export_to_ntopology(self.session.samples, self.heatmap.get_normalized_grid(), 
                            self.heatmap.width_mm, self.heatmap.height_mm, "export_ntop.json")

    def export_obj(self):
        if not self.session.samples:
            print("No data to export")
            return
        export_heightfield_obj(self.heatmap.get_normalized_grid(), 
                               self.heatmap.width_mm, self.heatmap.height_mm, output_path="export_relief.obj")
    ...
    def export_heatmap_png(self):
        ...
        if source_frame is not None:
            ...
            QMessageBox.information(self, "Export Complete", 
                f"Saved at {width}x{height}:\n\n"
                f"• Transparent overlay:\n  {heatmap_only_path}\n\n"
                f"• Combined with source:\n  {combined_path}")
        else:
            QMessageBox.information(self, "Export Complete", 
                f"Saved at {width}x{height}:\n\n"
                f"• Transparent overlay:\n  {heatmap_only_path}")
